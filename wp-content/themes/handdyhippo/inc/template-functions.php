 ...
 * @return array
 */
function handdyhippo_body_classes( $classes ) {
...
add_filter( 'body_class', 'handdyhippo_body_classes' );
 ...
 * @return array
 */
function handdyhippo_post_classes( $classes ) {
	$classes[] = 'entry';

	return $classes;
}
add_filter( 'post_class', 'handdyhippo_post_classes', 10, 3 );
 ...
 * @return void
 */
function handdyhippo_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'handdyhippo_pingback_header' );
 ...
 * @return void
 */
function handdyhippo_supports_js() {
	echo '<script>document.body.classList.remove("no-js");</script>';
}
add_action( 'wp_footer', 'handdyhippo_supports_js' );
 ...
 * @return array
 */
function handdyhippo_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'handdyhippo_comment_form_defaults' );
 ...
 * @return bool
 */
function handdyhippo_can_show_post_thumbnail() {
	/**
	...
	return apply_filters(
		'handdyhippo_can_show_post_thumbnail',
 ...
 * @return int
 */
function handdyhippo_get_avatar_size() {
	return 60;
}
 ...
 */
function handdyhippo_continue_reading_text() {
	$continue_reading = sprintf(
		/* translators: %s: Post title. Only visible to screen readers. */
		esc_html__( 'Continue reading %s', 'handdyhippo' ),
 ...
 */
function handdyhippo_continue_reading_link_excerpt() {
	if ( ! is_admin() ) {
		return '&hellip; <a class="more-link" href="' . esc_url( get_permalink() ) . '">' . handdyhippo_continue_reading_text() . '</a>';
	}
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'handdyhippo_continue_reading_link_excerpt' );
 ...
 */
function handdyhippo_continue_reading_link() {
	if ( ! is_admin() ) {
		return '<div class="more-link-container"><a class="more-link" href="' . esc_url( get_permalink() ) . '#more-' . esc_attr( get_the_ID() ) . '">' . handdyhippo_continue_reading_text() . '</a></div>';
	}
}

// Filter the content more link.
add_filter( 'the_content_more_link', 'handdyhippo_continue_reading_link' );

if ( ! function_exists( 'handdyhippo_post_title' ) ) {
	/**
	 ...
	 * @return string
	 */
	function handdyhippo_post_title( $title ) {
		return '' === $title ? esc_html_x( 'Untitled', 'Added to posts and pages that are missing titles', 'handdyhippo' ) : $title;
	}
}
add_filter( 'the_title', 'handdyhippo_post_title' );
 ...
 * @return string
 */
function handdyhippo_get_icon_svg( $group, $icon, $size = 24 ) {
	return handdyhippo_SVG_Icons::get_svg( $group, $icon, $size );
}
 ...
 * @return string
 */
function handdyhippo_change_calendar_nav_arrows( $calendar_output ) {
	$calendar_output = str_replace( '&laquo; ', is_rtl() ? handdyhippo_get_icon_svg( 'ui', 'arrow_right' ) : handdyhippo_get_icon_svg( 'ui', 'arrow_left' ), $calendar_output );
	$calendar_output = str_replace( ' &raquo;', is_rtl() ? handdyhippo_get_icon_svg( 'ui', 'arrow_left' ) : handdyhippo_get_icon_svg( 'ui', 'arrow_right' ), $calendar_output );
	return $calendar_output;
}
add_filter( 'get_calendar', 'handdyhippo_change_calendar_nav_arrows' );
 ...
 * @return string
 */
function handdyhippo_get_non_latin_css( $type = 'front-end' ) {
	...
	$font_family = apply_filters(
		'handdyhippo_get_localized_font_family_types',
		array(
	...
	$elements = apply_filters(
		'handdyhippo_get_localized_font_family_elements',
		array(
	...
	if ( ! function_exists( 'handdyhippo_generate_css' ) ) {
	...
	return handdyhippo_generate_css( // @phpstan-ignore-line.
 ...
 */
function handdyhippo_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
 ...
 * @since handdyhippo1.4 Corrected parameter name for `$output`,
 ...
 */
function handdyhippo_password_form( $output, $post = 0 ) {
	$post   = get_post( $post );
	$label  = 'pwbox-' . ( empty( $post->ID ) ? wp_rand() : $post->ID );
	$output = '<p class="post-password-message">' . esc_html__( 'This content is password protected. Please enter a password to view.', 'handdyhippo' ) . '</p>
	<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
	<label class="post-password-form__label" for="' . esc_attr( $label ) . '">' . esc_html_x( 'Password', 'Post password form', 'handdyhippo' ) . '</label><input class="post-password-form__input" name="post_password" id="' . esc_attr( $label ) . '" type="password" size="20" /><input type="submit" class="post-password-form__submit" name="' . esc_attr_x( 'Submit', 'Post password form', 'handdyhippo' ) . '" value="' . esc_attr_x( 'Enter', 'Post password form', 'handdyhippo' ) . '" /></form>
	';
	return $output;
}
add_filter( 'the_password_form', 'handdyhippo_password_form', 10, 2 );
 ...
 */
function handdyhippo_get_attachment_image_attributes( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'handdyhippo_get_attachment_image_attributes', 10, 3 );
